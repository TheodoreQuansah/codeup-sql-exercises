...
SELECT *
FROM dept_emp
WHERE from_date LIKE (SELECT hire_date
FROM employees 
WHERE emp_no LIKE '101010') AND to_date > CURDATE();
...
SELECT CONCAT(employees.first_name, ' ', employees.last_name) AS female_managers, gender 
FROM employees 
...
FROM salaries) AND salaries.to_date > CURDATE()
ORDER BY salary ASC;
...
FROM salaries 
WHERE to_date > CURDATE())
AND to_date > CURDATE()) / (SELECT count(salary)
FROM salaries
WHERE to_date > CURDATE())
) * 100 AS perc_of_sal;
